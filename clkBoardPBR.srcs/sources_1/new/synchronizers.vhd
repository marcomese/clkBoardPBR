...
acqStatusSmplInst: xpm_cdc_single
generic map (
    DEST_SYNC_FF   => 2,
    INIT_SYNC_FF   => 1,
    SIM_ASSERT_CHK => 0,
...
port map (
    ...
    dest_clk => clkSmpl,
    src_in   => acqStatusIn,
    dest_out => acqStatusSampled
);

acqStatusSyncInst: xpm_cdc_single
generic map (
    DEST_SYNC_FF   => 2,
    INIT_SYNC_FF   => 1,
    SIM_ASSERT_CHK => 0,
...
port map (
    src_clk  => clkSmpl,
    dest_clk => clk,
    src_in   => acqStatusSampled,
    dest_out => acqStatusSync
);
...
generic map (
    DEST_SYNC_FF   => 5, -- 3 additional 200MHz register to cope with the delay with respect to the trgIn
    INIT_SYNC_FF   => 1,
    SIM_ASSERT_CHK => 0,
    SRC_INPUT_REG  => 0,
    WIDTH          => zynqNum
...
extTrgSmplInst: xpm_cdc_array_single
generic map (
    DEST_SYNC_FF   => 2,
    INIT_SYNC_FF   => 1,
    SIM_ASSERT_CHK => 0,
